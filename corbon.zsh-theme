...
CORBON_WIDTH_THRESHOLD="${CORBON_WIDTH_THRESHOLD:-80}"
...
_corbon_cache_valid() {
  local key="$1"
  local ttl="$2"
  local timestamp="${CORBON_CACHE_TIME[$key]:-0}"

  (( SECONDS - timestamp < ttl ))
}
...
_corbon_cache_set() {
  ...
  CORBON_CACHE[$key]="$value"
  CORBON_CACHE_TIME[$key]=$SECONDS
}

_corbon_cache_get() {
  print -r -- "${CORBON_CACHE[$1]:-}"
}
...
_corbon_git_status() {
  ...
  git_output="$(command git status --porcelain=v1 2>/dev/null)" || return
  ...
  (( conflict > 0 )) &&
    result+=" $(_corbon_symbol git_conflict)$conflict"

  (( staged > 0 )) &&
    result+=" $(_corbon_symbol git_staged)$staged"

  (( unstaged > 0 )) &&
    result+=" $(_corbon_symbol git_dirty)$unstaged"

  (( untracked > 0 )) &&
    result+=" $(_corbon_symbol git_untracked)$untracked"
...
_corbon_git_ahead_behind() {
  ...
  (( behind > 0 )) &&
    _corbon_paint git_behind " $(_corbon_symbol git_behind)$behind"

  (( ahead > 0 )) &&
    _corbon_paint git_ahead " $(_corbon_symbol git_ahead)$ahead"
}

_corbon_segment_git() {
  local root branch git_state ahead

  root="$(_corbon_git_root)" || return

  if _corbon_cache_valid "git:$root" "$CORBON_GIT_CACHE_TTL"; then
    git_state="$(_corbon_cache_get "git:$root")"
  else
    git_state="$(_corbon_git_status)"
    _corbon_cache_set "git:$root" "$git_state"
  fi
  ...
  if [[ -z "$git_state" ]]; then
    ...
    _corbon_style "${CORBON_STYLE[git_clean]}"
...
_corbon_segment_context() {
  ...
  hostname_value="$(hostname -s 2>/dev/null)"
  ...
  if [[ "$CORBON_SHOW_HOST" == true ]]; then
    [[ -n "$output" ]] && output+="@"
    output+="$hostname_value"
  fi

  if [[ -n "$SSH_CONNECTION" || -n "$SSH_TTY" ]]; then
    output+=" $(_corbon_symbol ssh)"
  fi

  if [[ "$EUID" == 0 ]]; then
    output+=" $(_corbon_symbol root)"
  fi

  if [[ "$CORBON_SHOW_CONTAINER" == true ]] &&
     [[ -f /.dockerenv || -f /run/.containerenv ]]; then
    output+=" $(_corbon_symbol container)"
  fi

  [[ -n "$output" ]] || return

  _corbon_paint accent "$output"
}
...
_corbon_runtime_version() {
  ...
  if _corbon_cache_valid "runtime:$key" "$CORBON_RUNTIME_CACHE_TTL"; then
    _corbon_cache_get "runtime:$key"
    return
  fi
  ...
  _corbon_cache_set "runtime:$key" "$version"
...
_corbon_runtime_segment() {
  ...
  version="$(_corbon_runtime_version "$key")"
  [[ -z "$version" ]] && return

  _corbon_paint "$color" " $(_corbon_symbol "$symbol") $version"
}

_corbon_segment_python() {
  _corbon_runtime_segment python python python
}

_corbon_segment_node() {
  _corbon_runtime_segment node node node
}

_corbon_segment_go() {
  _corbon_runtime_segment go go go
}

_corbon_segment_rust() {
  _corbon_runtime_segment rust rust rust
}

_corbon_segment_java() {
  _corbon_runtime_segment java java java
}

_corbon_segment_ruby() {
  _corbon_runtime_segment ruby ruby ruby
}
...
_corbon_segment_terraform() {
  ...
  local version
  version="$(terraform version 2>/dev/null |
    head -1 |
    awk '{print $2}' |
    sed 's/^v//')"

  _corbon_paint terraform \
    " $(_corbon_symbol terraform) ${version:-terraform}"
}

_corbon_segment_aws() {
  [[ -n "$AWS_PROFILE" ||
     -n "$AWS_DEFAULT_REGION" ]] || return

  _corbon_paint aws \
    " $(_corbon_symbol aws) ${AWS_PROFILE:-default}"
}

_corbon_segment_gcp() {
  [[ -n "$CLOUDSDK_ACTIVE_CONFIG_NAME" ||
     -n "$GOOGLE_CLOUD_PROJECT" ]] || return

  _corbon_paint gcp \
    " $(_corbon_symbol gcp) ${GOOGLE_CLOUD_PROJECT:-gcp}"
}

_corbon_segment_azure() {
  [[ -n "$AZURE_SUBSCRIPTION_ID" ||
     -n "$AZURE_DEFAULTS_GROUP" ]] || return

  _corbon_paint azure \
    " $(_corbon_symbol azure) azure"
}
...
_corbon_segment_os() {
  ...
  _corbon_paint os \
    " $(_corbon_symbol os) $os"
}

_corbon_segment_arch() {
  _corbon_paint arch \
    " $(_corbon_symbol arch) $(uname -m)"
}

_corbon_segment_shell() {
  _corbon_paint shell \
    " $(_corbon_symbol shell) zsh $ZSH_VERSION"
}
...
_corbon_segment_jobs() {
  ...
  _corbon_paint jobs \
    " $(_corbon_symbol jobs)$count"
}

_corbon_segment_time() {
  _corbon_paint time \
    " $(_corbon_symbol time) $(date +%H:%M)"
}

_corbon_segment_date() {
  _corbon_paint date \
    " $(_corbon_symbol date) $(date +%Y-%m-%d)"
}
...
_corbon_segment_duration() {
  ...
  _corbon_paint duration \
    " $(_corbon_symbol duration) ${elapsed}s"
}
...
_corbon_segment_exit() {
  ...
  _corbon_paint error " $(_corbon_symbol git_conflict) $code" bold
}
...
_corbon_render_segment() {
  ...
  case "$name" in
    ...
    python)
      [[ "$CORBON_SHOW_PYTHON" == true ]] &&
        _corbon_segment_python
      ;;
    node)
      [[ "$CORBON_SHOW_NODE" == true ]] &&
        _corbon_segment_node
      ;;
    go)
      [[ "$CORBON_SHOW_GO" == true ]] &&
      ...
      ;;
    rust)
      [[ "$CORBON_SHOW_RUST" == true ]] &&
        _corbon_segment_rust
      ;;
    java)
      [[ "$CORBON_SHOW_JAVA" == true ]] &&
        _corbon_segment_java
      ;;
    ruby)
      [[ "$CORBON_SHOW_RUBY" == true ]] &&
        _corbon_segment_ruby
      ;;

    docker)
      [[ "$CORBON_SHOW_DOCKER" == true ]] &&
        _corbon_segment_docker
      ;;
    kubernetes)
      [[ "$CORBON_SHOW_KUBERNETES" == true ]] &&
        _corbon_segment_kubernetes
      ;;
    terraform)
      [[ "$CORBON_SHOW_TERRAFORM" == true ]] &&
        _corbon_segment_terraform
      ;;
    aws)
      [[ "$CORBON_SHOW_AWS" == true ]] &&
        _corbon_segment_aws
      ;;
    gcp)
      [[ "$CORBON_SHOW_GCP" == true ]] &&
        _corbon_segment_gcp
      ;;
    azure)
      [[ "$CORBON_SHOW_AZURE" == true ]] &&
        _corbon_segment_azure
      ;;
    ...
    os)
      ...
      ;;
    arch)
      [[ "$CORBON_SHOW_ARCH" == true ]] &&
        _corbon_segment_arch
      ;;
    shell)
      [[ "$CORBON_SHOW_SHELL" == true ]] &&
        _corbon_segment_shell
      ;;
    battery)
      [[ "$CORBON_SHOW_BATTERY" == true ]] &&
        _corbon_segment_battery
      ;;
    jobs)
      ...
      ;;
    time)
      [[ "$CORBON_SHOW_TIME" == true ]] &&
        _corbon_segment_time
      ;;
    date)
      [[ "$CORBON_SHOW_DATE" == true ]] &&
        _corbon_segment_date
      ;;
    duration)
      _corbon_segment_duration
      ;;
    exit)
      _corbon_segment_exit "$exit_code"
      ;;
    custom:*)
      _corbon_render_custom "${name#custom:}"
      ;;
    *)
...
_corbon_apply_preset() {
  case "$1" in
    ...
    power)
      CORBON_LEFT=(context path git)
      CORBON_RIGHT=(
        python node go rust java ruby
        docker kubernetes terraform
        aws gcp azure
        duration exit jobs time
      )
      ;;
...
_corbon_adapt() {
  if (( COLUMNS < CORBON_WIDTH_THRESHOLD )); then
    CORBON_ADAPTIVE_RIGHT=(
      ${CORBON_RIGHT:#runtime}
      ${CORBON_RIGHT:#time}
      ${CORBON_RIGHT:#date}
    )
  else
    CORBON_ADAPTIVE_RIGHT=("${CORBON_RIGHT[@]}")
  fi
}
...
_corbon_precmd() {
  ...
  if [[ "$CORBON_LAYOUT" == one-line ]]; then
    _corbon_render_side left "$exit_code"
    print -Pn " "
    _corbon_render_side right "$exit_code"
  ...
  if (( exit_code != 0 )); then
    PROMPT="$(
      _corbon_paint error \
        "$(_corbon_symbol git_conflict) $exit_code" bold
    )$PROMPT"
  fi

  if [[ "$CORBON_LAYOUT" == two-line ]]; then
    right="$(
      for segment in "${CORBON_ADAPTIVE_RIGHT[@]}"; do
        _corbon_render_segment "$segment" "$exit_code"
      done
    )"
...
_corbon_init_segments() {
  ...
  for segment in \
    ...
    docker kubernetes terraform \
    aws gcp azure \
    os arch shell battery jobs \
    time date duration exit
  do
...
_corbon_install_hooks() {
  if (( ${preexec_functions[(I)_corbon_preexec]} == 0 )); then
    preexec_functions+=(_corbon_preexec)
  fi

  if (( ${precmd_functions[(I)_corbon_precmd]} == 0 )); then
...
_corbon_init() {
  _corbon_init_segments
  _corbon_detect_color_mode
  _corbon_apply_preset "${CORBON_PRESET:-}"
  _corbon_ls_colors
  _corbon_install_hooks
